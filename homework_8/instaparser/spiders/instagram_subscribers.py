...
class InstagramSubscribersSpider(scrapy.Spider):
    ...
    subscribers_hash = 'c76146de99bb02f6415203be841dd25a'  # hash для получения списка подписок
    ...
    def user_subscriptions_parse(self, response: HtmlResponse, username):
        ...
        url_subscriptions = f'{self.graphql_url}query_hash={self.subscribers_hash}&{urlencode(variables)}'  # Формируем ссылку для получения данных о постах
        yield response.follow(
            url_subscriptions,
    ...
    def user_subscriptions_continue(self, response: HtmlResponse, username, user_id,
        ...
        subscribers = j_data.get('data').get('user').get('edge_followed_by').get('edges')  # Подписки
        for subscriber in subscribers:  # Перебираем посты, собираем данные
